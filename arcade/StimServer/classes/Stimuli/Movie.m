classdef Movie < Stimulus
    % MOVIE - Class for movie stimuli to be passed to the StimServer.
    %
    % Usage
    % -----
    %   mov = Movie(filename);
    ...
    methods
        function obj = Movie(filename)
            ...
            assert(exist(filename, 'file') == 2, 'Movie %s file not found', filename)
            [~,~,ext] = fileparts(filename);
%            validExts = {'.png', '.bmp', '.jpeg', '.jpg'};
%            validatestring(ext, validExts);                        
            
            StimServer.Command(0, uint8([6 filename 0]));
            obj = obj@Stimulus();

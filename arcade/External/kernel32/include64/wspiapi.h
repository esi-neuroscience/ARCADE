...
typedef int (WINAPI *WSPIAPI_PGETNAMEINFO) (const struct sockaddr *sa,socklen_t salen,char *host,size_t hostlen,char *serv,size_t servlen,
	 int flags);
typedef void (WINAPI *WSPIAPI_PFREEADDRINFO) (struct addrinfo *ai);
_inline 
char *
WINAPI WspiapiStrdup (const char * pszString) {
	char *pszMemory;
	size_t cchMemory;
	if (!pszString)
		return(NULL);
...
WINAPI WspiapiNewAddrInfo (int iSocketType,int iProtocol,WORD wPort,DWORD dwAddress) {
...
__inline int WINAPI WspiapiLegacyGetNameInfo(const struct sockaddr *ptSocketAddress,socklen_t tSocketLength,char *pszNodeName,size_t tNodeLength,char *pszServiceName,
...
__inline int WINAPI WspiapiGetNameInfo (const struct sockaddr *sa,socklen_t salen,char *host,size_t hostlen,char *serv,size_t servlen,
	 int flags) {
	int iError;

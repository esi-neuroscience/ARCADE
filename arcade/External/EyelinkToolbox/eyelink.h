...
/*! @ingroup access_time_local
	Returns the current millisecond since the initialization.
 	...
 	@remarks If the eyelink_exptkit library is not initialized, or initialized multiple times,
 				the return value is invalid and the return value is unpredictable. So in order to avoid
				this, make sure that \c close_eyelink_system() is called at the end. 
				The call to \c current_msec() is always equivalent to \c current_time().
...
\code
Example 2:
	// "HTARGET" should be included in the data list
	...
	// The remote data is accessed through the hdata member of the sample data structure. 
	// The first 4 values of evt.hdata structure are: target x, target y, target distance in mm*10.0, and target flags.
...
/*! @ingroup init_eyelink
	...
	@return \c 0 if not connected, \c 1 for EyeLink I, \c 2 for EyeLink II \c 3 for EyeLink 1000.
